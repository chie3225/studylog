...
module.exports = async (req, res) => {
  ...
  const system =
    ...
    '写真には、生徒が自分で答え合わせをして手書きで○や×(またはレ点、チェックなど)をつけた形跡があるはずです。' +
    'その手書きの印だけを根拠に、各問題が○だったか×だったかを読み取ってください。' +
    '写真のどこにも丸つけの形跡(○や×などの手書きの印)が見当たらない場合は、marksDetectedをfalseにし、itemsは空配列にしてください。' +
  ...
  try {
    const rawText = await callClaude({ system, imageBase64: image_base64, text: contextText, maxTokens: 2000 });
